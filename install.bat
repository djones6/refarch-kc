...
helm install order-query-ms ../refarch-kc-order-ms/order-query-ms/chart/orderqueryms --wait --timeout=300s --set image.repository=ibmcase/kcontainer-order-query-ms --set kafka.brokersConfigMap=kafka-brokers --set eventstreams.enabled=false --set eventstreams.truststoreRequired=false --set serviceAccountName=kcserviceaccount --namespace shipping

:: Install spring-container-ms using pre-built image
:: note: uses postgres secrets defined above
helm install spring-container-ms ..\refarch-kc-container-ms\SpringContainerMS\chart\springcontainerms --set image.repository=ibmcase/kcontainer-spring-container-ms --set kafka.brokersConfigMap=kafka-brokers --set eventstreams.enabled=false --set eventstreams.truststoreRequired=false --set serviceAccountName=kcserviceaccount --namespace shipping
